...
app.get("/api/Students", (req, res) => {
  const studentsToSend = students.map(
...
app.get("/api/Students/:id", ({ params: { id } }, res) =>
  res.send(
    students
...
app.put(
  "/api/Students/:id",
  ({ params: { id }, body: { firstName, lastName, dateOfBirth } }, res) => {
...
app.get(
  "/api/Students/:studentID/Nationality/",
...
app.put(
  "/api/Students/:studentID/Nationality/:nationalityID",
  ({ params: { studentID, nationalityID } }, res) => {
    const studentIndexToUpdate = students.findIndex(
      ({ ID }) => ID === parseInt(studentID, 10)
    );
    const nationalityToUpdate = nationalities.find(
      ({ ID }) => ID === parseInt(nationalityID, 10)
    );
...
app.get(
  "/api/Students/:studentID/FamilyMembers/",
  ({ params: { studentID } }, res) => {
    const familyMembersForStudent = students.find(
      (s) => s.ID === parseInt(studentID, 10)
    ).familyMembers;
...
app.post("/api/Students/:studentID/FamilyMembers/", (req, res) => {
  const {
    body: { firstName, lastName, dateOfBirth, relationship },
    params: { studentID },
  } = req;
  const studentToUpdate = students.find(
    (s) => s.ID === parseInt(studentID, 10)
  ...
  studentToUpdate.familyMembers = (studentToUpdate.familyMembers || []).concat([
    newFamilyMember,
  ]);
...
app.put("/api/FamilyMembers/:id", (req, res) => {
  ...
  const studentToUpdate = students.find((s) =>
    s.familyMembers.some((k) => k.ID === parseInt(ID, 10))
  );
  const familyMemberToUpdate = studentToUpdate.familyMembers.find(
    (s) => s.ID === parseInt(ID, 10)
  );
  const studentIndex = students.indexOf(studentToUpdate);
  const familyMemberIndex =
    studentToUpdate.familyMembers.indexOf(familyMemberToUpdate);
  ...
  students[studentIndex].familyMembers[familyMemberIndex] =
...
app.get(
  "/api/FamilyMembers/:familyMemberID/Nationality/",
  ({ params: { familyMemberID } }, res) => {
    const studentToUpdate = students.find((s) =>
      s.familyMembers.some((k) => k.ID === parseInt(familyMemberID, 10))
    );
    const familyMemberToUpdate = studentToUpdate.familyMembers.find(
      (s) => s.ID === parseInt(familyMemberID, 10)
    );
...
app.put(
  "/api/FamilyMembers/:familyMemberID/Nationality/:nationalityID",
  ({ params: { familyMemberID, nationalityID } }, res) => {
    ...
    students[studentIndex].familyMembers[familyMemberIndex].nationality =
      nationality;
